...
class ClusterCountingAlgorithmSteppable(SteppableBasePy):
    ...
    def start(self):
        ...
        # place mesenchymal cells random in space
        ...
        # set volume properties of all cells identical
        ...
        write_clusters, file_path = self.open_file_in_simulation_output_folder('data_clusters_N%03d_kappa%04d_tau%04d_frac%03d_1000gEE%04d_1000gEM%04d_10000gMM%05d.dat'%(int(Np),int(kappa),int(tau),int(frac*100),int(gEE*1000),int(gEM*1000),int(gMM*10000)), mode='w')
        if write_clusters is None:
            return   
           
        write_cells, file_path = self.open_file_in_simulation_output_folder('data_cells_N%03d_kappa%04d_tau%04d_frac%03d_1000gEE%04d_1000gEM%04d_10000gMM%05d.dat'%(int(Np),int(kappa),int(tau),int(frac*100),int(gEE*1000),int(gEM*1000),int(gMM*10000)), mode='w')
        ...
        write_metadata, file_path = self.open_file_in_simulation_output_folder('metadata_N%03d_kappa%04d_tau%04d_frac%03d_1000gEE%04d_1000gEM%04d_10000gMM%05d.dat'%(int(Np),int(kappa),int(tau),int(frac*100),int(gEE*1000),int(gEM*1000),int(gMM*10000)), mode='w')
        ...
        print('focus of active cells:', frac,file=write_metadata) #should say: fraction of active cells
    ...
    def step(self,mcs):
        # define polarity direction after equilibration
        if mcs == 2500:
            for cell in self.cell_list:
            ...
            for cell in self.cell_list:
                ...
                for neighbor, common_surface_area in self.get_cell_neighbor_data_list(cell):
                    if neighbor:
                        ...
                        if neighbor.type == self.EPITHELIAL and cell.type == self.MESENCHYMAL:    
                            Pix += gEM*np.cos(neighbor.dict['angle_pol_now'])
                            Piy += gEM*np.sin(neighbor.dict['angle_pol_now'])
                        if neighbor.type == self.MESENCHYMAL and cell.type == self.EPITHELIAL:    
                            Pix += gEM*np.cos(neighbor.dict['angle_pol_now'])
                            Piy += gEM*np.sin(neighbor.dict['angle_pol_now'])
                        if neighbor.type == self.EPITHELIAL and cell.type == self.EPITHELIAL:    
                            Pix += gEE*np.cos(neighbor.dict['angle_pol_now'])
                            Piy += gEE*np.sin(neighbor.dict['angle_pol_now'])
